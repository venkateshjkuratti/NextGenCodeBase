ImgVisionApp.controller('myWorkItemViewController', function ($rootScope, $scope, $log, $http, $window, $uibModal, $routeParams, $sce, $location, urlConstants, commonServices, myWorkItemBasketServices, uploadServices, adminServices, userDetailsServices) {
    ...
    myWorkItemBasketServices.getData().then(function () {

        //checking whether the details comes from basket page or workflow page
        if (pageType == workFlowViewEmitter.basket) {
            itemData = myWorkItemBasketServices.myData();
        } else
            itemData = JSON.parse(localStorage.getItem('getAdminQueueWorkItemsDetails'));
        //fetching the routing buttons for the selected instances
        for (i = 0; i < itemData.length; i++) {
            if (itemData[i].InstanceId == instanceId) {
                $scope.routingButtons = itemData[i].TransitionRules;
                console.log($scope.routingButtons);
                break;
            }
        }
    }, function () {
    ...
    $scope.isRoutingButton = function () {
        return localStorage.getItem('setUserPerferenceRoutingButton') == 'button';
    }
    ...
    userDetailsServices.getUserSystemId().then(function (response) {
        ...
        selectFields = urlConstants.workItemIndexingFields + $rootScope.userDetails.UserName + '/' + systemId + '/' + docTypeName + '/' + $routeParams.storageRepoId;
