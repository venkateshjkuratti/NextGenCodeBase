...
angular.module("ngTableResize").directive('resizeable', ['resizeStorage', '$injector', function(resizeStorage, $injector) {
    ...
    function initialiseAll(table, attr, scope) {
        ...
        cache = resizeStorage.loadTableSizes(table, "OverflowResizer")
    ...
    function getResizer(scope) {
        try {
            var Resizer = $injector.get("OverflowResizer")
            return Resizer;
        } catch (e) {
            console.error("The resizer was not found");
            return null;
        }
    }
